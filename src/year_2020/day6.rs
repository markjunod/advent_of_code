...
pub fn run() {
    ...
    info!("2020 - Day 6 - Part 1: Took {} millis", part1_millis);

    let part2_millis = time_millis(|| run_part2());

    info!("2020 - Day 6 - Part 2: Took {} millis", part2_millis);
}

fn run_part1() {
    let sum_of_yesses_counts: u32 = ALL_ANSWERS.iter().map(|answers| count_yesses_in_group(answers)).sum();

    info!("2020 - Day 6 - Part 1: Sum of anyone answering yes is {}", sum_of_yesses_counts);
}

fn run_part2() {
    let sum_of_yesses_counts: u32 = ALL_ANSWERS.iter().map(|answers| count_overlapping_yesses(answers)).sum();

    info!("2020 - Day 6 - Part 2: Sum of everyone answering yes is {}", sum_of_yesses_counts);
}
